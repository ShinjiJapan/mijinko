...
- `FilePreview.IsEditable(kind)`= Text/Markdown/Code/Html が true(画像・PDF・None は不可)。`FilePreview.HasRenderedPreview(kind)`= Markdown/Html/Code が true(編集中の逆ペインプレビュー可否)。
- `FilePreview.PreviewKeyHints(kind, sourceMode, editKey, toggleKey)`= プレビュー情報バー併記用のキーヒント文字列を組み立てる純粋関数。編集可能なら `(editKey:編集)`、レンダリング ⇄ ソース切替可(`HasRenderedPreview`)なら `(toggleKey:プレビュー/ソース)`(現在表示の逆を表示。sourceMode 中は「プレビュー」、レンダリング中は「ソース」)。テスト: `FilePreviewTests`(分類・各 helper を網羅)。
