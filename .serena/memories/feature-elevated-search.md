...
- `src/Filer.App/FileSearchDialog.xaml(.cs)` — 非管理者(proxy 非 null)時のみ「高速検索(管理者権限が必要)」ボタン表示。`Search_Click`=通常走査/`FastSearch_Click`=proxy 経由、共通の `StartSearchAsync(elevated)`→`RunSearchAsync(options, elevated)` で分岐。`ElevationDeclinedException` は理由表示のみ(ボタン維持)。コンストラクタ `FileSearchDialog(baseDir, ElevatedSearchProxy? proxy=null)`。
- `src/Filer.App/MainWindow.xaml.cs` — `GetOrCreateSearchProxy()`(管理者判定 `WindowsPrincipal.IsInRole(Administrator)`、管理者なら null=ボタン出さない。非管理者は単一 `ElevatedSearchProxy` を使い回しヘルパー常駐)。`ShowFileSearchDialog` で dialog へ渡す。`OnClosed` で `_searchProxy?.Dispose()`。
